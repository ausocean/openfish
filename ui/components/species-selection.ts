...
export class SpeciesSelection extends TailwindElement {
  ...
  private selectSpecies(species: Species | null) {
  ...
  render() {
    ...
    if (!this._loading) {
      if (this._speciesList.length > 0) {
        results = html`
          <ul class="grid overflow-hidden gap-4 p-4 grid-cols-2 auto-rows-auto">
            ${repeat(
              this._speciesList,
              (species) => species.id,
              (species) => html`
    ...
    return html`
      ...
      >
        <input
          type="search"
          class="bg-blue-700 border border-blue-800 text-blue-50 w-full rounded-md placeholder:text-blue-300 mb-1"
          placeholder="Search species"
          @input=${this.search}
        />
        <button class="mx-auto btn bg-rose-600 text-white my-5 text-xl hover:ring-2 hover:ring-sky-400/50 data-selected:ring-2 data-selected:ring-sky-400 data-selected:ring-offset-4 ring-offset-1 ring-offset-blue-700 transition-shadow rounded-md" @click=${() => this.selectSpecies(null)} ?data-selected=${this.selection === null}>I don't know what this is</button>
      </header>
      <div class="relative overflow-y-scroll h-[calc(100%-3rem)]">
        <div class="absolute inset-0">
          ...
          <footer class="w-full pb-4">
            ${
              this._loading
                ? html`<p class="text-2xl text-center text-white mt-5">
                  Loading...
                </p>`
                : nothing
            }
            ${
              !this._loading && this._isMore
                ? html` <button
                  class="mx-auto btn variant-slate"
